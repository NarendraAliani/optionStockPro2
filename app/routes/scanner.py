...
@scanner_bp.route('/api/live/logs/latest-csv', methods=['GET'])
@login_required
def download_latest_live_log_csv_scanner():
    """Compatibility route for downloading latest live cycle CSV log."""
    logs_dir = Path('logs')
    if not logs_dir.exists() or not logs_dir.is_dir():
        return jsonify({'success': False, 'error': 'Log directory not found.'}), 404
